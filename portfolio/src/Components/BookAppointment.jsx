...
const BookAoopintment=()=>{
    ...
    const availableTimes=[
        ...
        '12:00PM','02:00PM','03:00PM','04:00PM'
    ];
    ...
    <div className='max-w-4xl mx-auto'>
        <div className='text-center mb-12'>
            ...
            <p className='text-lg text-gray-600'>Loream ipsum dolor sit amet consectetur adipisicing elit.</p>
        </div>
        <div className='mb-12'>
            <div className='relative'>
                <div className='absolute top-1/2 left-0 right-0 h-2 bg-gray-100 rounded-full z-0'></div>
                <div className='absolute top-1/2 left-0 h-2 bg-gradient-to-r from-sky-400 to-sky-400 
                        ...
                        onClick={()=>activeStep > step && setActiveStep(step)}>
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${activeStep>=step
                                ...
                                transition-all duration-300 font-bold mb-2`}>
                                    {activeStep>step?(
                                        <svg className='w-5 h-5' fill='none' stroke='currentColor' viewBox='0 0 14 24'>
                                            <path strokeLinecap='round'
                                            strokeLinejoin='round'
                                            strokeWidth='2'
                                            d='MS 1314 4L19 7'></path>
                                        </svg>
                                    ):(
                                        step
                                    )}
                            ...
                            <span className={`text-xs font-medium ${activeStep>=step
                                ?'text-sky-600':'text-gray-400'} transition-colors`}>
        ...
        <div className='bg-white rounded-3xl shadow-xl overflow-hidden'>
                ...
                <div className='p-8'>
                    ...
                    < div className='space-y-6'>
                        <div className='relative'>
                            ...
                            focus:border-sky-5000 focus:ring-sky-2000 outline-none transition' required/>
                            <FaUser className='absolute left-4 top-1/2 transform -transition-y-1/2 text-gray-400'/>
                        </div>
                        <div className='relative'>
                            ...
                            className='w-full p-4 lp-12 border-2 border-gray-200 rounded-xl focus:ring-2 
                            focus:border-sky-5000 focus:ring-sky-2000 outline-none transition' required/>
                            <FaPhone className='absolute left-4 top-1/2 transform -transition-y-1/2 text-gray-400'/>
                        </div>
                    </div>
                </div>
            )}
            {activeStep===2 && (
                <div className='p-8'>
                    <h2 className='text-2xl font-bold text-gray-800 mb-6 flex items-center'>
                        <FaCalendarAlt className='mr-3 text-sky-500'/>Appoinment Details
                    </h2>
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                        <div className='reltive'>
                            ...
                            focus:border-sky-500 focus:ring-2 focus-sky-200 outline-none' required
                            min={new Date().toISOString().split('T')[0]}/>
                            <FaCalendarAlt className='absolute left-4 top-1/2 transform -transition-y-1/2 text-gray-400'/>
                        </div>
                        <div className='relative'>
                            <select name="time" 
                            value={formData.time}
                            ...
                            focus:border-sky-500 focus:ring-2 focus:ring-sky-200 outline-none appearance-none transition' required>
                            ...
                            <FaClock className='absolute left-4 top-1/2 transform -transition-y-1/2 text-gray-400'/>
                            <FaChevronRight className='absolute left-4 top-1/2 transform -transition-y-1/2 text-gray-400'/>
                        </div>
                        <div className='relative md:col-span-2'>
                            ...
                            <FaChevronRight className='absolute left-4 top-1/2 transform -transition-y-1/2 rotate-90 text-gray-400'/>
                        </div>
                    </div>
                </div>
            )}
               {activeStep=== 3&&(
                ...
                <button className='w-full max-w-xs bg-gradient-to-r from-sky-700 to-sky-700
                text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all transform
                hover:scale-205'>Confirm & Book Appointment</button>
                </div>
               )}
               <div className='px-8 pb-8 flex justify-between'>
                {activeStep > 1 && (
                    <button onClick={prevStep}
                    className='px-6 py-3 text-gray-600 font-medium rounded-lg hover:bg-gray-100 transition'>Back
                    </button>
                )}
                {activeStep < 3 && (
                    <button 
...
export default BookAoopintment